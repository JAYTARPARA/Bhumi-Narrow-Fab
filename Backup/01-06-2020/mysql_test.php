...
if (isset($_REQUEST['callapi'])) {
    ...
    if ($process == "sendNotification") {
        // sendCustomNotification($_REQUEST, $con);
        sendSNSFlutter($con);
    } else if ($process == "addUser") {
...
function registerUser($data, $con)
{
    ...
    echo $sql    = "SELECT id FROM users WHERE mobile='$mobile'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        echo $sql = "INSERT INTO users (mobile, password, name, address, gst) VALUES ('$mobile', '$password', '$name', '', '$gst')";
...
function sendCustomNotification($data, $con)
{
    $url          = "https://fcm.googleapis.com/fcm/send";
    $token        = "cgw7260aSqqlf1_n7SiVbp:APA91bHpG-91c-KnPvqCyt6XNgjZfX90iRflnTTzEUeaGbfliqcwuARuoQoe8p1VtFbqYjwKknBxQ6UpTYLGQllcWeLN1aOYH3c52xcapd-f640CJobWfpoF463ox_aTQ6nxcb-dOLw2";
    $serverKey    = 'AAAATnQdvgk:APA91bGXFqpNIqdG8WfIA_Ngg6Qs-Ui0J30huazmVD8kS3Emb__loDLCKpV4jkSSiNba0EhLcJxZD_hSec6bsjs6eu7RByCzMKhpqoG38wiXIQtgvvhwDp_EOT5gD21PiyM8ZV5jodbu';
    $title        = "New Material Uploaded";
    $body         = "Hey, New material available. Please click here to see.";
    $notification = array('title' => $title, 'body' => $body, 'sound' => 'default', 'badge' => '1', 'channelId' => 'com.fl.app.bhuminarrowfab');
    $arrayToSend  = array('to' => $token, 'notification' => $notification, 'priority' => 'high');
    $json         = json_encode($arrayToSend);
    $headers      = array();
    $headers[]    = 'Content-Type: application/json';
