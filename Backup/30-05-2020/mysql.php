...
if (isset($_REQUEST['callapi'])) {
    ...
    } else if ($process == "registerUser") {
        registerUser($_REQUEST, $con);
    } else if ($process == "loginUser") {
        loginUser($_REQUEST, $con);
    ...
    } else if ($process == "getSliderMaterials") {
      getSliderMaterials($_REQUEST, $con);
    ...
    } else if ($process == "chkUserStatus") {
        chkUserStatus($_REQUEST, $con);
    } else if ($process == "updateProfile") {
        updateProfile($_REQUEST, $con);
    } else if ($process == "chkProfile") {
        chkProfile($_REQUEST, $con);
    } else if ($process == "saveToken") {
        saveToken($_REQUEST, $con);
    } else if ($process == "getPassword") {
        getPassword($_REQUEST, $con);
    }
}
...
function registerUser($data, $con)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $sql = "INSERT INTO users (mobile, password, name, address, gst) VALUES ('$mobile', '$password', '$name', '', '$gst')";
...
function loginUser($data, $con)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        $sqlPass    = "SELECT id, status FROM users WHERE mobile='$mobile' AND password='$password'";
...
function chkUserStatus($data, $con)
{
    $id = $data['id'];
...
function updateProfile($data, $con)
{
    $id       = $data['id'];
    $name     = $data['name'];
    $mobile   = $data['mobile'];
    ...
    $sql = " UPDATE users SET $updatePassword name='$name', gst='$gst', address='$address', profile_status=1 WHERE id=$id ";
...
function chkProfile($data, $con)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            if ($row['mobile'] == "" || $row['name'] == "" || $row['address'] == "" || $row['gst'] == "") {
                $result = array(
                    'success'          => 1,
                    'profileCompleted' => 0,
                    'message'          => "User found.",
...
function getMaterials($data, $con)
{
    ...
    if ($type != "All") {
        $searchTypeQry = " AND material_type='$type' ";
    }

    if ($search != "") {
        $searchQry = " AND ( material_id LIKE '%$search%' OR material_type LIKE '%$search%' OR name LIKE '%$search%' OR price LIKE '%$search%' OR color LIKE '%$search%' ) ";
    }
    ...
    $sqlCount    = "SELECT id as COUNT FROM materials WHERE status=1 $searchQry $searchOwnerQry $searchTypeQry ";
    $resultCount = mysqli_query($con, $sqlCount);
    $totalCount  = mysqli_num_rows($resultCount);

    $sql    = "SELECT * FROM materials WHERE status=1 $searchQry $searchOwnerQry $searchTypeQry $sortQry $limitQry ";
    $result = mysqli_query($con, $sql);
...
function getSliderMaterials($data, $con)
{
    $materials = array();

    $sortQry = " ORDER BY id DESC ";

    $sqlCount    = "SELECT id as COUNT FROM materials WHERE status=1 AND slider=1 ";
    $resultCount = mysqli_query($con, $sqlCount);
    $totalCount  = mysqli_num_rows($resultCount);

    $sql    = "SELECT * FROM materials WHERE status=1 AND slider=1 $sortQry ";
    $result = mysqli_query($con, $sql);
...
function saveOrder($data, $con)
{
    ...
    $sql = "INSERT INTO orders (user_id, order_id, mobile, material_id, quantity, price, sample, piece, status, created_at) VALUES ($userid, '$orderid', '$usermobile', $materialprimaryid, $materialquantity, '$materialprice', $matsample, $piece, 0, '$date')";
...
function getOrderByID($data, $con)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $unit              = $explodeunit[0];
            if ($row['quantity'] == 0) {
                $row['totalprice'] = round($row['piece'] * $unitprice);
            } else {
                $row['totalprice'] = round(($row['quantity'] * $unitprice) / $unit);
            }
            $row['quantity'] = $row['quantity'] . 'M';
...
function uploadMaterial($data, $filedata, $con)
{
    ...
    if (move_uploaded_file($filedata['photo']['tmp_name'], $target_path)) {
        $sql = "INSERT INTO materials (image, name, price, sort_price, material_id, material_type, color, company) VALUES ('$img', '$name', '$price', $sort_price, '$mid', '$material_type', '$color', '$mowner')";
        if (mysqli_query($con, $sql)) {
            // sendNotification();
            sendSNS($con);
            sendSNSFlutter($con);
...
function updateMaterial($data, $filedata, $con)
{
    ...
    if (move_uploaded_file($filedata['photo']['tmp_name'], $target_path)) {
        ...
        $sql = " UPDATE materials set image='$img', name='$name', price='$price', sort_price='$sort_price', material_id='$mid', color='$color', company='$mowner', material_type='$mtype' WHERE id=$id ";
        if (mysqli_query($con, $sql)) {
            $result = array(
                'success' => 1,
...
function updateMaterialDetailOnly($data, $con)
{
    ...
    $sql = " UPDATE materials set name='$name', price='$price', sort_price='$sort_price', material_type='$mtype', material_id='$mid', color='$color', company='$mowner' WHERE id=$id ";
    if (mysqli_query($con, $sql)) {
        $result = array(
            'success' => 1,
            'message' => "Material updated successfully!!!",
...
function updateOrderStatus($data, $con)
{
    ...
    if (mysqli_query($con, $sql)) {
        sendOrderUpdateNotification($con, $id, $status);
        $result = array(
            'success' => 1,
...
function blockUnblockUser($data, $con)
{
    ...
    if (mysqli_query($con, $sql)) {
        sendUserUpdateNotification($con, $mobile, $status);
        $result = array(
            'success' => 1,
            'message' => $successMsg,
        );
...
function saveToken($data, $con)
{
    $id             = $data['id'];
    $token          = $data['token'];
    $registrationId = base64_decode($data['token']);

    $sql    = "UPDATE users SET token='$registrationId' WHERE id=$id ";
    $result = mysqli_query($con, $sql);
...
function sendSNSFlutter($con)
{
    $sql    = "SELECT name, status, token FROM `users` WHERE status = 1 AND token != '' ";
    $result = mysqli_query($con, $sql);
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $url          = "https://fcm.googleapis.com/fcm/send";
            $token        = $row['token'];
            $serverKey    = 'AAAATnQdvgk:APA91bGXFqpNIqdG8WfIA_Ngg6Qs-Ui0J30huazmVD8kS3Emb__loDLCKpV4jkSSiNba0EhLcJxZD_hSec6bsjs6eu7RByCzMKhpqoG38wiXIQtgvvhwDp_EOT5gD21PiyM8ZV5jodbu';
            $title        = "New Material avaialable";
            $body         = "Hey " . ucfirst(explode(" ", $row["name"])[0]) . ", New material available. Please click here to see.";
            $notification = array('title' => $title, 'text' => $body, 'sound' => 'default', 'badge' => '1');
            $arrayToSend  = array('to' => $token, 'notification' => $notification, 'priority' => 'high');
            $json         = json_encode($arrayToSend);
            $headers      = array();
...
function getPassword($data, $con)
{
    $mobile = $data['mobile'];

    $sql    = "SELECT password FROM users WHERE mobile='$mobile'";
    $result = mysqli_query($con, $sql);

    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $password = $row['password'];
            $headers  = "From: 20.jaystar@gmail.com";
            $response = mail("91$mobile@sms.textlocal.in", "Password Reset", "Your password is: $password", $headers);
            if ($response == 1) {
                $result = array(
                    'success' => 1,
                    'message' => "We have sent a password on $mobile, Please wait for some time",
                );
            } else {
                $result = array(
                    'success' => 0,
        ...
        $result = array(
            'success' => 0,
            'message' => "You are not registred with this mobile number",
        );
    }
...
function sendOrderUpdateNotification($con, $id, $status)
{
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $uid         = $row['user_id'];
            $sqlToken    = "SELECT token, name FROM `users` WHERE id = '$uid' ";
            $resultToken = mysqli_query($con, $sqlToken);
            if (mysqli_num_rows($resultToken) > 0) {
                while ($rowToken = mysqli_fetch_assoc($resultToken)) {
                    ...
                    if ($status == 0) {
                        $statusMsg = "Pending";
                        $fullMsg   = "Your order number $id is $statusMsg";
                    } else if ($status == 1) {
                        $statusMsg = "Confirmed";
                        $fullMsg   = "Your order number $id is $statusMsg";
                    } else if ($status == 2) {
                        ...
                        $statusMsg = "Rejected";
                        $fullMsg   = "Your order number $id is $statusMsg. You can contact to the owner.";
                    }
                    ...
                    $title        = "Order Updated";
                    $data         = ["orderid" => $id];
                    $body         = "Hey " . ucfirst(explode(" ", $uname)[0]) . ", $fullMsg";
                    $notification = array('title' => $title, 'text' => $body, 'sound' => 'default', 'badge' => '1');
...
function sendUserUpdateNotification($con, $mobile, $status)
{
    $sql    = "SELECT token, name FROM `users` WHERE mobile = '$mobile' ";
    $result = mysqli_query($con, $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $uname    = $row['name'];
            ...
            $titleMsg = "";
            if ($status == 0) {
                $titleMsg = "You are blocked";
                $fullMsg  = "You are blocked by admin and you can not access the app now.";
            } else if ($status == 1) {
                $titleMsg = "You are unblocked";
                $fullMsg  = "You are unblocked by admin and you can access the app now.";
            }
