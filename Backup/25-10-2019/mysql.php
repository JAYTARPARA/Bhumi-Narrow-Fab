...
function updateUser($data, $con) {
	...
	$sql = " UPDATE users SET name='$name', gst='$gst', address='$address' WHERE mobile='$mobile' ";
...
function getMaterials($data, $con) {
	...
	if ($search != "") {
		$searchQry = " AND material_id LIKE '%$search%' OR name LIKE '%$search%' OR price LIKE '%$search%' ";
	}
	...
	$sql = "SELECT * FROM materials WHERE 1 AND status=1 $searchQry $searchOwnerQry ORDER BY id DESC $limitQry ";
	$result = mysqli_query($con, $sql);
...
function uploadMaterial($data, $filedata, $con) {
	...
	if (move_uploaded_file($filedata['photo']['tmp_name'], $target_path)) {
		$sql = "INSERT INTO materials (image, name, price, material_id, company) VALUES ('$img', '$name', '$price', '$mid', '$mowner')";
		if (mysqli_query($con, $sql)) {
			$result = array(
				'success' => 1,
...
function updateMaterial($data, $filedata, $con) {
	...
	if (move_uploaded_file($filedata['photo']['tmp_name'], $target_path)) {
		...
		$sql = " UPDATE materials set image='$img', name='$name', price='$price', material_id='$mid' WHERE id=$id ";
		if (mysqli_query($con, $sql)) {
			$result = array(
				'success' => 1,
...
function updateMaterialDetailOnly($data, $con) {
	...
	$sql = " UPDATE materials set name='$name', price='$price', material_id='$mid' WHERE id=$id ";
	if (mysqli_query($con, $sql)) {
		$result = array(
			'success' => 1,
			'message' => "Material updated successfully!!!",
