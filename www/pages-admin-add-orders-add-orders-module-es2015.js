...
let AddOrdersPage = class AddOrdersPage {
    ...
    addOrder() {
        ...
        if (partyname == undefined || partymobile == undefined || partyaddress == undefined || quantity == undefined || partyname == '' || partymobile == '' || partyaddress == '' || quantity == '') {
            ...
            // tslint:disable-next-line:max-line-length
            fileTransfer.upload(this.base64Image, `https://bhuminarrowfab.000webhostapp.com/mysql.php?callapi=1&process=addOrder&partyname=${partyname}&partymobile=${partymobile}&partyaddress=${partyaddress}&quantity=${quantity}&pieces=${pieces}&sample=${sample}`, options).then(result => {
